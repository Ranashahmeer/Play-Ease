...
/*
  Local Court type (keeps everything self-contained)
*/
type Court = {
  id?: string;
  name: string;
...
export class OurServicesComponent implements OnInit, OnDestroy {
  ...
  // transient fields for building court
  pitches: string[] = [];
  offers: string[] = [];
  images: string[] = [];
  mainImage: string = '';

  // per-pitch pricing map
  pricePerPitch: Record<string, number> = {};
  ...
  // validation / UX
  submitted = false; // set true when user attempts to save - used to show validation messages

  // popup for viewing a saved court
  popupVisible = false;
  ...
  /**
   * DevExtreme ClickEvent wrapper for Edit button.
   * It contains the DOM event at ev.event; stopPropagation there.
   */
  ...
  /**
   * DevExtreme ClickEvent wrapper for Delete button.
   */
  ...
  saveCourt() {
    ...
    // collect missing fields for message
    ...
    if (missing.length > 0) {
      // show user what to fill
      alert(`Please fill the following fields before saving:\n• ${missing.join('\n• ')}`);
      return;
    }

    // build cleaned pricePerPitch and min price
    const cleaned: Record<string, number> = {};
    for (const p of this.pitches) {
      cleaned[p] = Math.round(this.pricePerPitch[p]);
    }
  ...
  /**
   * Small helper to show an icon/emoji for well-known offers.
   */
